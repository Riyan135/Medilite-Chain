...
import { KeyRound, ShieldCheck, ShieldOff, UserCheck, UserPlus } from 'lucide-react';
    ...
    <div className="flex min-h-screen bg-[#f0f4f8]">
      ...
      <main className="flex-1 overflow-y-auto px-4 pb-8 pt-20 md:px-8 md:pt-8">
        ...
        <section className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
          ...
          <AccessStat title="Doctor IDs Issued" value={doctors.filter((user) => user.doctorId).length} icon={KeyRound} tone="amber" />
        </section>
        ...
        <div className="grid grid-cols-1 gap-8 xl:grid-cols-[1.05fr_0.95fr]">
          <section className="rounded-[2rem] border border-slate-100 bg-white shadow-sm overflow-hidden">
            <div className="border-b border-slate-100 px-6 py-5">
              <h2 className="text-2xl font-black text-slate-900">Doctor Provisioning Rules</h2>
              <p className="mt-1 text-sm text-slate-500">Doctors are issued from User Governance and log in only with their Doctor ID.</p>
            </div>

            <div className="divide-y divide-slate-100">
              {doctors.length === 0 ? (
                <div className="p-8 text-slate-400">No doctor accounts have been provisioned yet.</div>
              ) : (
                doctors.map((doctor) => (
                  <div key={doctor.id || doctor._id} className="flex flex-col gap-4 px-6 py-5 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-xs font-black uppercase tracking-[0.18em] text-blue-600">
                        {doctor.doctorId || 'Doctor ID missing'}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black uppercase tracking-wider text-emerald-700">Provisioned</span>
                    </div>
                  </div>
                ))
              )}
          ...
          <section className="space-y-6">
            <Panel
              ...
              items={[
                ...
                'Doctors access consultations, appointments, and clinical workflows only after a Doctor ID is issued.',
            ...
            <Panel
              ...
              items={[
                ...
                'Doctor accounts must be created from User Governance instead of self-sign-up.',
